...
if [ "X$QUEUE" != "X" ]; then
    if [ $CLI != "start-scala-shell.sh" ]; then
       echo "executing $EXECUTABLE ${KEEP[@]:0:3} -yqu $QUEUE ${KEEP[@]:3}"
       exec "$EXECUTABLE" `echo "${KEEP[@]:0:3} -yqu $QUEUE ${KEEP[@]:3}"`
    else
       echo "executing $EXECUTABLE ${KEEP[@]:0:1} -qu $QUEUE ${KEEP[@]:1}"
       exec "$EXECUTABLE" `echo "${KEEP[@]:0:1} -qu $QUEUE ${KEEP[@]:1}"`
    fi
    echo "executing $EXECUTABLE ${KEEP[@]:0:3} -yqu $QUEUE ${KEEP[@]:3}"
    exec "$EXECUTABLE" `echo "${KEEP[@]:0:3} -yqu $QUEUE ${KEEP[@]:3}"`
else
